...
capitanes_fourfactors_26feb23 <- read.csv('data/capitanes_vs_clippers_26feb23_fourfactors.csv')

#Generate a gt table from head of data
head(capitanes_fourfactors_26feb23) %>% 
  gt()

(ff_table <- head(capitanes_fourfactors_26feb23) %>% 
    gt() %>%
