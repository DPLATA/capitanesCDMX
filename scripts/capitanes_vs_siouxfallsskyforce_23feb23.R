...
capitanes_fourfactors_23feb23 <- read.csv('data/capitanes_vs_skyforce_23feb23_fourfactors.csv')

#Generate a gt table from head of data
head(capitanes_fourfactors_23feb23) %>% 
  gt()

(ff_table <- head(capitanes_fourfactors_23feb23) %>% 
    gt() %>%
    ...
    tab_header(title = md("CAPITANES VS SKY FORCE")) %>% 
