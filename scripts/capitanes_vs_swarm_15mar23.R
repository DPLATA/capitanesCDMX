...
capitanes_fourfactors_15mar23 <- read.csv('data/capitanes_vs_swarm_15mar23_fourfactors.csv')

#Generate a gt table from head of data
head(capitanes_fourfactors_15mar23) %>% 
  gt()

(ff_table <- head(capitanes_fourfactors_15mar23) %>% 
    gt() %>%
    ...
    tab_header(title = md("CAPITANES VS SWARM")) %>% 
